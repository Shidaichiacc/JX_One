...
function SimCore:AbortNew(nListId)
    local tbNpc = self.fighterList[nListId]
    if not tbNpc then return end
    if tbNpc.finalIndex and tbNpc.finalIndex > 0 then DelNpcSafe(tbNpc.finalIndex) end
...
function SimCore:RetrySpawn(tbNpc, rate)
    if not tbNpc.spawnRetryTick then return 0 end
    tbNpc.tick_breath = (tbNpc.tick_breath or 0) + (rate or 1)
    if tbNpc.tick_breath < tbNpc.spawnRetryTick then return 1 end
    if (tbNpc.spawnRetryCount or 0) >= (SIMBOT_RESPAWN_MAX_RETRIES or 3) then
        self:Remove(tbNpc.id)
        return 1
    end
    tbNpc.spawnRetryCount = (tbNpc.spawnRetryCount or 0) + 1
    if tbNpc.entitySys:CreateChar(self, tbNpc, 0, tbNpc.goX32, tbNpc.goY32) ~= 0 then
        tbNpc.spawnRetryTick = nil
        tbNpc.spawnRetryCount = 0
    else
        tbNpc.spawnRetryTick = tbNpc.tick_breath + (SIMBOT_RESPAWN_RETRY_TICKS or 5*18/REFRESH_RATE)
    end
    return 1
end

function SimBotRecoverStuck(simInstance, tbNpc)
    ...
    if tbNpc.tick_breath < tbNpc.tick_checklag then return 0 end
    ...
    if moved then
        ...
        tbNpc.stuckWatchX = x; tbNpc.stuckWatchY = y
    ...
    if tbNpc.stuckRetries > (SIMBOT_STUCK_MAX_RETRIES or 2) then
        tbNpc.stuckRetries = 0
        tbNpc.tick_checklag = nil
        tbNpc.entitySys:Respawn(simInstance, tbNpc, 4, "simbot stuck")
        return 1
    end

    tbNpc.movementSys:resetPos(simInstance, tbNpc.id)
    tbNpc.stuckWatchX = x; tbNpc.stuckWatchY = y
    tbNpc.tick_checklag = tbNpc.tick_breath + (SIMBOT_STUCK_CHECK_TICKS or 30*18/REFRESH_RATE)
    return 1
end
...
function SimCore:OnTimer(tbNpc, rate)
    ...
    -- FIX: RetrySpawn existed but was never called from anywhere, so
    -- tbNpc.spawnRetryTick (set by sim.entity.lua on a failed respawn) was
    -- never actually processed. Dispatch it here, before anything else that
    -- assumes tbNpc.finalIndex is valid.
    if (not tbNpc.finalIndex or tbNpc.finalIndex <= 0) and tbNpc.isDead ~= 1 and tbNpc.spawnRetryTick then
        return self:RetrySpawn(tbNpc, rate)
    end
    ...
    if (tbNpc.finalIndex and tbNpc.finalIndex > 0 and tbNpc.isDead ~= 1) then
        if SimBotRecoverStuck(self, tbNpc) == 1 then return 0 end
...
-- [NEW FEATURE] Safety-net sweep: removes any fighter left in a state that
-- should never legitimately persist -- no valid finalIndex, not marked dead,
-- and no pending spawn retry (SimCore:RetrySpawn). Every known death/respawn
-- path (entitySys.OnDeath, entitySys.Respawn, SimBotRecoverStuck) always
-- leaves a fighter in one of: alive with a valid finalIndex, isDead==1
-- waiting to revive, or spawnRetryTick set waiting for RetrySpawn. Landing
-- outside all three means some code path failed to clean up after itself --
-- exactly the class of bug already found and fixed this session (a bot that
-- silently vanishes from the map but keeps being ticked forever). Call this
-- periodically (see refillLoop in main.lua) rather than every tick, since it
-- is a diagnostic safety net, not part of the normal lifecycle.
function SimCore:SweepStaleGhosts()
    local swept = 0
    for id, fighter in self.fighterList do
        if (not fighter.finalIndex or fighter.finalIndex <= 0)
           and fighter.isDead ~= 1
           and not fighter.spawnRetryTick then
            self:Remove(id)
            swept = swept + 1
        end
    end
