...
function OnDeath(nBossIndex)
	...
	if GetTeamSize() >= 2 then
		for k=1,GetTeamSize() do
			...
			AddPlayerExp(KinhNghiemGietBoss)
	...
	if tbplayer and getn(tbplayer) > 0 then
		for k=1,getn(tbplayer) do
			...
			AddPlayerExp(KinhNghiemNguoiChoiDungXungQuanh)
	...
	--DropGoldBossRandomEquip(nBossIndex, OrgPlayer)    --- luon luon rot 3 4 mon hkmp
...
GOLD_BOSS_RANDOM_EQUIP_POOL = nil

function GoldBoss_AddRandomEquipPool(tbPool, tbUsed, nGoldGenre, nStartId, nEndId)
...
function GoldBoss_BuildRandomEquipPool()
	local tbPool = {}
	local tbUsed = {}

	for _, tbBossAward in BIGBOSS_AWARD do
		if tbBossAward.tbItemIDTime then
			for _, nGoldId in tbBossAward.tbItemIDTime do
				GoldBoss_AddRandomEquipPool(tbPool, tbUsed, 0, nGoldId, nGoldId)
			end
		end
	end
...
function DropGoldBossRandomEquip(nNpcIndex, OrgPlayer)
	if not GOLD_BOSS_RANDOM_EQUIP_POOL then
		GOLD_BOSS_RANDOM_EQUIP_POOL = GoldBoss_BuildRandomEquipPool()
	end

	if not GOLD_BOSS_RANDOM_EQUIP_POOL or getn(GOLD_BOSS_RANDOM_EQUIP_POOL) <= 0 then
		return
	end
	...
	for i = 1, nDropCount do
		local tbItem = GOLD_BOSS_RANDOM_EQUIP_POOL[random(getn(GOLD_BOSS_RANDOM_EQUIP_POOL))]
		local itemidx = AddDropGoldItem(nNpcIndex, tbItem)
		if itemidx and itemidx > 0 then
			_LogPlayer("award","gold_boss",{GetLocalDate("%m/%d/%Y_%H:%M:%S"),format("%s rãi ra tè boss %s",GetItemName(itemidx),szBossName),format("t¹i map: %s to¹ ®é (%d,%d)",SubWorldName(SubWorld),floor((nX32/32)/8),floor( (nY32/32) / 16) )})
		end
	end
end
